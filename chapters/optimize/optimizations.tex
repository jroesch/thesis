...
The full set of rules can be found in the \autoref{appx:hetero-rules}.
...
In these cases symbolic codegen cannot completely replace manually tuned 3rd party libraries yet,
  but is complimentary when partial shapes are known.\yida{Can we remove this paragraph?}
...
% When an operator appears with dynamic shape it often will only vary along a single dimension.
...
% We reuse the machinery of the VM to support invoking a bucketed operator.
