...
In fact Relay has a definitional interpreter written in
  C++ which implements exactly this strategy,
  a recursive AST traversal.
A definitional interpreter is a good way to
  specify the behavior of program, but not
  to efficiently execute them.
The program in its current form is not
  suitable to efficient execution and
  must be further transformed.
Finally the dynamic behaviors must
  be accounted for and some require
  runtime support.
...
We describe the general backend compilation strategy,
  then lowering Relay to the graph runtime, the virtual machine,
  ahead of time compiler, and finally VTA a hardware accelerator
  built by my collaborators.
In this chapter we evaluate state-of-the-art NLP
  applications and demonstrate state-of-the-art performance
  beating leading industry standards such as TensorFlow and
  PyTorch.
...
Processes such as automatic differentiation can be performed
  during this step.
Next we extract so called primitive functions
  from the Relay program, the process for selecting these
  is defined in Section \ref{sec:fusion}.
These are lowered into TIR the low-level IR of TVM.
Relay then further transforms the remaining code
  around the primitive functions in multiple passes
  described below.
...
Finally we lower the remaining Relay code to one of our runtimes,
  either the interpreter, which directly interprets the AST,
  the virtual machine which requires more compilation steps
  as described below,
  the graph runtime,
  or to native code.
